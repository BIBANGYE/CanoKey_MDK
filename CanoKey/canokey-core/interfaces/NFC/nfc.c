...
static volatile enum { TO_RECEIVE,
                       TO_SEND } next_state;
...
void nfc_send_frame(uint8_t prologue, uint8_t *data, uint8_t len)
{
    ...
    {
        if (device_spinlock_lock(&state_spinlock, true) != 0)
            return;
...
static void send_apdu_buffer(uint8_t resend)
{
    ...
    if (apdu_buffer_tx_size - apdu_buffer_sent > last_sent)
        prologue |= PCB_I_CHAINING;
    nfc_send_frame(prologue, global_buffer + apdu_buffer_sent, last_sent);
    apdu_buffer_sent += last_sent;
    if (apdu_buffer_tx_size == apdu_buffer_sent)
        inf_sending = 0;
}
